...
// Updated: 29-FEB-2017 Handeling GET&POST parameters <mirabedini@gmail.com>
//
...
boolean TinyWebServer::process_parameters(char* buffer)
{
  int i = 0,l=strlen(buffer)+1;
  if (parameters_) {
    free(parameters_);
    parameters_ = NULL;
  }
  ...
  if(j==0) return true;
  parameters_ = (HeaderValue*)malloc_check(sizeof(HeaderValue) * (j + 1));
  int state=0;
  j=0;
  for(i=0;i<l;i++){
    if(state==0){
      parameters_[j].header=buffer+i;
      parameters_[j].value=NULL;
      state=1;
    }
    if(state==2){
      parameters_[j].value=buffer+i;
      state=3;
    }
    if(state==1 && buffer[i]=='='){
      state=2;buffer[i]=0;
    }
    if(state==3 && (buffer[i]=='&' || i==l-1)){
      state=0;
      buffer[i]=0;
#if DEBUG
      Serial << "\r\n Parameter :" << parameters_[j].header << parameters_[j].value;
#endif
      j++;
    }
  }
  parameters_[j].header=NULL;
  parameters_[j].value=NULL;
  return true;
}
...
boolean TinyWebServer::process_headers() {
  ...
  while (1) {
    ...
    if (state == END_HEADERS) {
      ...
      if(request_type_ == HttpRequestType::POST){
          const char* value=get_header_value("Content-Length");
          if(value){
            int i=atoi(value);
            char* params = (char*)malloc_check(sizeof(char) * (i + 1));
            pos=0;
            while(read_next_char(client_, (uint8_t*)&ch)){
              params[pos++]=ch;
            }
            params[pos]=0;
...
const char* TinyWebServer::get_parameter_value(const char* name) {
  if (!parameters_) {
    return NULL;
  }
  for (int i = 0; parameters_[i].header; i++) {
    if (!strcmp(parameters_[i].header, name)) {
