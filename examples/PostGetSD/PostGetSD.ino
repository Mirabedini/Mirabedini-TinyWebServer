...
boolean post_handler(TinyWebServer& web_server) {
  ...
  free(filename);
  char* pLed=web_server.get_parameter_value("LED");
  Serial << "\r\n pLed : " << strlen(pLed) << " - " << pLed <<"\r\n";
  if(!strcmp("ON",pLed)){
    Serial << "\r\nOn command\r\n";
    digitalWrite(Led, HIGH);
  }
  else if(!strcmp("OFF",pLed)){
    Serial << "\r\nOFF command\r\n";
...
void setup() {
  ...
  // LED
  pinMode(Led, OUTPUT); // set the SS pin as an output (necessary!)
  digitalWrite(Led, LOW); // but turn off the W5100 chip!
